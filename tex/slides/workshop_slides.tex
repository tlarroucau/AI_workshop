...
\begin{frame}{The Symbiosis: Agents as Builders}
    ...
    \begin{columns}[c]
        \begin{column}{0.4\textwidth}
            ...
            \begin{tikzpicture}
                % The Dinosaur
                \node [scale=4] (dino) at (0,0) {\twemoji{t-rex}};
                \node [scale=1.5] at (0.8, 0.8) {\twemoji{construction}};
                
                % The Electric Fence (Grid)
                \draw[gray!70, ultra thick] (2,-1.5) -- (2,1.5);
                \draw[gray!70, ultra thick] (3.5,-1.5) -- (3.5,1.5);
                \foreach \y in {-1, 0, 1}
                    \draw[gray, thick] (1.8,\y) -- (3.7,\y);
                
                % Post-its on the fence (Memory/Docs)
                \node[fill=yellow!80, draw=yellow!50!black, rotate=5, minimum width=0.6cm, minimum height=0.5cm, drop shadow] at (2, 0.5) {\tiny\texttt{\#42}};
                \node[fill=pink!80, draw=pink!50!black, rotate=-10, minimum width=0.6cm, minimum height=0.5cm, drop shadow] at (3.5, -0.5) {\tiny\texttt{PR}};
                
                % Action
                \draw[->, ultra thick, blue!60!black, dashed] (0.8,0) -- (1.8,0);
            \end{tikzpicture}
            
            \vspace{0.4cm}
            \small\textit{"The dinosaurs help build the park."}
        \end{column}
        ...
        \begin{column}{0.6\textwidth}
            ...
            \begin{itemize}
                ...
                \item \textbf{GitHub $\rightarrow$ Memory} \\
...
\begin{columns}[t]
    ...
    \begin{column}{0.48\textwidth}
        ...
        \begin{itemize}
            ...
            \item \textbf{Copilot}: Install "GitHub Copilot" extension (Required for Module I \& II)
            \item \textbf{MCP Server}: Will be configured in Module II (Agentic AI)
        \end{itemize}
...
\begin{enumerate}
    \item \textbf{Ask Mode (Chat Panel)} \twemoji{t-rex}
    \begin{itemize}
    ...
    \item \textbf{Edit Mode (Inline)} \scalebox{1.5}{\twemoji{t-rex}}
    \begin{itemize}
...
\begin{enumerate}
    \setcounter{enumi}{2}
    \item \textbf{Agent Mode (@workspace)} \scalebox{2.5}{\twemoji{t-rex}}
    \begin{itemize}
    ...
    \item \textbf{Plan Mode (New!)} \twemoji{t-rex}
    \begin{itemize}
...
\begin{frame}{VS Code Custom Context}
    \textbf{Tailoring Copilot to your Project:}
    \vspace{0.2cm}
    \begin{itemize}
        \item \textbf{Custom Instructions} (\texttt{.github/copilot-instructions.md}): 
        \begin{itemize}
            \item High-level context: project architecture, coding patterns, tech stack.
            \item Always appended to the system prompt.
        \end{itemize}
        \item \textbf{Prompt Files} (\texttt{.github/prompts/*.prompt.md}):
        \begin{itemize}
            \item Reusable, pre-defined prompts for specific tasks (e.g., "Summarize", "Refactor").
            \item Can specify a particular model (e.g., GPT-4o, Claude 3.5 Sonnet).
            \item Added to the user prompt when invoked.
        \end{itemize}
        \item \textbf{Custom Agents} (Configured in settings):
        \begin{itemize}
            \item Define specific personas or workflows (e.g., "Planner", "Implementer").
            \item Augment or override default agent behavior.
        \end{itemize}
    \end{itemize}
    \end{frame}
    
    \begin{frame}{Advanced Agent Capabilities: Skills}
    \textbf{Agent Skills (\texttt{.github/skills/})}
    \begin{itemize}
        \item Bundle scripts, templates, and instructions into a modular "skill".
        \item Enables the agent to perform complex actions (e.g., fetching data, running analysis).
        \item \textbf{Progressively loaded:} Only loaded into context when the agent decides to use them.
    \end{itemize}
    
    \vspace{0.5cm}
    
    \textbf{Demo: Custom Context in Action}
    \begin{enumerate}
        \item Reviewing \texttt{.github/copilot-instructions.md} (Project Standards)
        \item Using \texttt{.github/prompts/summarize.prompt.md} (Reusable Prompt)
        \item Triggering \texttt{.github/skills/youtube-transcript/} (fetching a video transcript)
    \end{enumerate}
    \end{frame}

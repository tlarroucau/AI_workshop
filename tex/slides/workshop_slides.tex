    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.33\textwidth}
            ...
            \onslide<3->{
                ...
                \begin{tikzpicture}
                    \node[rectangle, draw=ASUDarkGray, fill=ASULightGray, very thick, minimum size=2.5cm, minimum width=2.5cm, align=center] {Safety\\System};
                \end{tikzpicture}\\
                \vspace{0.3cm}
...
\textbf{Extension} & \textbf{Purpose} \\
\midrule
\multicolumn{2}{l}{\textit{Version Control \& GitHub}} \\
...
\begin{itemize}\setlength{\itemsep}{2pt}
    ...
    \item Install/discover via \texttt{npx skills.sh} (install Node first)
\end{itemize}
\end{column}
\end{columns}
...
\begin{frame}{Custom Agents in VS Code}
\begin{columns}[t]
\begin{column}{0.48\textwidth}
\textbf{What Are Custom Agents?}
\begin{itemize}\setlength{\itemsep}{3pt}
    \item User-defined agent \textbf{personas} that control how the AI behaves
    \item Appear in the chat mode dropdown alongside Ask / Edit / Agent
    \item \textbf{Plan mode} is itself a built-in custom agent!
\end{itemize}

\vspace{0.3cm}
\textbf{Structure of a Custom Agent:}
\begin{itemize}\setlength{\itemsep}{3pt}
    \item \textbf{Name} \& description
    \item \textbf{Tools} it can access
    \item \textbf{Handoffs} to other agents
    \item Detailed \textbf{instructions} (system prompt)
\end{itemize}
\end{column}

\begin{column}{0.48\textwidth}
\textbf{How to Create One:}
\begin{enumerate}\setlength{\itemsep}{3pt}
    \item Chat dropdown $\rightarrow$ \textit{Configure Custom Agents}
    \item Choose scope: \textbf{User} (global) or \textbf{Workspace}
    \item Define name, tools, and behavior
    \item Save --- it appears in the dropdown instantly
\end{enumerate}

\vspace{0.3cm}
\textbf{Community Gallery:}
\begin{itemize}\setlength{\itemsep}{3pt}
    \item Browse \& install agents shared on GitHub
    \item Examples: code reviewer, research assistant, planner
\end{itemize}
\end{column}
\end{columns}

% \vspace{0.3cm}
% \textbf{Demo:} Create a \textit{Research Agent} custom agent with domain-specific tools \& instructions
\end{frame}
...
\begin{frame}[plain]
    ...
    \vspace{1.5cm}
    \begin{tikzpicture}
        \node[rectangle, draw=ASUDarkGray, fill=ASULightGray, very thick, minimum size=3cm, minimum width=3cm, align=center] {\Large Local\\Safety};
    \end{tikzpicture}
\end{frame}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/merge_conflict.png}
\end{center}
...
\begin{frame}[plain]
    ...
    \vspace{1.5cm}
    \begin{tikzpicture}
        \node[rectangle, draw=ASUDarkGray, fill=ASULightGray, very thick, minimum size=3cm, minimum width=3cm, align=center] {\Large Cloud\\Memory};
    \end{tikzpicture}
\end{frame}
...
\vspace{0.3cm}
\textbf{Demo:} Setting up a project board + Creating issue + Assigning to Copilot
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{Context7}: Up-to-date library documentation
    \item \textbf{Firecrawl}: Web scraping and data extraction
...
\begin{tikzpicture}[node distance=1.5cm, auto, thick, scale=0.85, every node/.style={scale=0.65}]
    ...
    \tikzstyle{tool} = [rectangle, draw, fill=ASUDarkGray!15, text width=5.5em, text centered, rounded corners, minimum height=2.5em]
...
\begin{frame}{Optimum Workflow: Overleaf + VS Code}
    ...
    \begin{tikzpicture}[node distance=4cm, auto, >=stealth, thick]
        ...
        % 2. GitHub -> Local
        \draw[->, ASUDarkGray] (github.south) -- node[right, font=\footnotesize] {2. Pull \texttt{main}} (local.north);

...
\begin{itemize}
    ...
    \item VS Code Chat modes, Cline + Continue (can be used offline!)
...
\begin{itemize}
    ...
    \item Goes beyond what agents can do natively in VS Code or browser
    \item Enables \textit{direct interaction} with APIs, not just generating code for you to run
\end{itemize}

\vspace{0.2cm}

\textbf{Key MCP Servers for Research Workflow:}

\scriptsize
\begin{itemize}
    \item \textbf{GitHub MCP}: Agent creates/updates issues, PRs, project boards directly \\
    \textit{Without MCP:} Agent suggests commands; you copy-paste into GitHub \\
    \textit{With MCP:} Agent executes GitHub actions automatically
    
    \item \textbf{Filesystem MCP}: Access files outside current VS Code workspace \\
    \textit{Use case:} Read data from external drives, sync with network folders
    
    \item \textbf{Brave Search MCP}: Structured web search with cited sources \\
    \textit{Use case:} Research literature, find datasets, validate claims
    
    \item \textbf{Fetch MCP}: Download and parse web content (papers, documentation) \\
    \textit{Use case:} Extract data from websites, scrape research repositories
\end{itemize}

% \vspace{0.2cm}

% \normalsize
% \textbf{Setup:} Install via \texttt{npx}, configure in Cline/Continue/Claude Desktop settings with API tokens
\end{frame}
...
\begin{center}
\Large
    \textbf{This ENTIRE workshop was created with ONE prompt and TWO hours of edits!}
\end{center}

...
% Page style with ASU colored rule
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{3pt}
\renewcommand{\headrule}{\hbox to\headwidth{\color{ASUMaroon}\leaders\hrule height \headrulewidth\hfill}}
\fancyfoot[C]{\thepage}
\setlength{\headheight}{14pt}

% Hyperlink colors using ASU palette
\hypersetup{colorlinks=true, linkcolor=ASUMaroon, citecolor=ASUMaroon, urlcolor=ASUMaroon}

% Title information
\title{Sample Research Paper: Treatment Effects Analysis\thanks{This is a template paper demonstrating reproducible research workflows with Git, GitHub, and Agentic AI.}}
\author{Your Name\\
W. P. Carey School of Business\\
Arizona State University\\
\texttt{yourname@email.edu}}
\date{\today}
...
Modern research increasingly relies on computational workflows that integrate data analysis, version control, and document preparation \citep{example2019}. This paper demonstrates a reproducible research template that combines Python for data analysis, LaTeX for document preparation, and Git/GitHub for version control and collaboration \citep{github2024}.

The primary research question examines the effect of a binary treatment on an outcome of interest, controlling for demographic and socioeconomic characteristics. While the data are synthetic, the workflow represents best practices applicable to real research projects \citep{example2020}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/outcome_by_treatment.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/scatter_income_outcome.pdf}
    \caption{Outcome vs Income by Treatment Status}
    \label{fig:scatter}
    \vspace{0.1cm}
    \footnotesize
    \emph{Note:} Maroon points and lines represent the control group, while gold points and lines represent the treatment group.
\end{figure}

\subsection{Robustness}

To assess the sensitivity of our results to potential within-group correlation, we re-estimate both models using cluster-robust standard errors at the region level. Table \ref{tab:robustness} presents the comparison between default and clustered standard errors.

\input{tables/regression_robustness.tex}

The clustered standard errors are somewhat larger than the default OLS standard errors, reflecting within-region correlation in the outcome variable. Importantly, the treatment effect remains statistically significant under both approaches, confirming the robustness of our main findings.

\subsubsection{Testing Overleaf integration}

If you are reading this paragraph it means that Overleaf is correctly integrated with the \texttt{main} branch!
...
This template was created for a workshop on "Git, GitHub, and VS Code: Agentic AI for Project Management and Research Productivity." Thanks to workshop participants for valuable feedback.
